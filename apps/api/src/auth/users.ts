...
import type { AuthenticatedUser, BackgroundTasks, Identity, Role } from './types';
...
// google_sub / display_name は補完の要否を判定するために読む（埋まっていれば UPDATE を出さない）。
const USER_COLUMNS = 'id, role, status, google_sub, display_name';
...
/** 行に空きがあり、かつ Identity が埋められる値を持っているか（補完 UPDATE の要否）。 */
const canFillProfile = (row: UserRow, profile: ProfileFields): boolean =>
  (row.google_sub === null && profile.googleSub !== null) ||
  (row.display_name === null && profile.displayName !== null);
 ...
 *
 * レスポンス内容に影響しない後処理なので、呼び出し側が waitUntil() で追跡する。
...
const acceptOrActivate = async (
  ...
  if (row.status === ACTIVE_STATUS) {
    ...
    if (user && canFillProfile(row, profile)) {
      // 補完は認証結果に影響しない。待たずに返し、waitUntil で完了だけ追跡する。
      executionCtx.waitUntil(fillMissingProfile(database, row.id, profile));
    }
    return user;
  }
...
export const resolveUser = async (
  ...
  if (identity.kind === 'access') {
    ...
    return acceptOrActivate(database, row, { googleSub: null, displayName: null }, executionCtx);
  }

  const bySub = await findByGoogleSub(database, identity.googleSub);
  if (bySub) {
    return acceptOrActivate(
      database,
      bySub,
      { googleSub: null, displayName: identity.displayName },
      executionCtx,
    );
  }
  ...
  return acceptOrActivate(
    database,
    byEmail,
    { googleSub: identity.googleSub, displayName: identity.displayName },
    executionCtx,
  );
};
