...
### コレクションと、それに対する計算を離さない

ある集合に対する計算（合計・最大・絞り込み）が画面ごとに書かれていたら、
**集合を受け取って結果を返す関数**へ寄せる。呼ぶ側が要素を1つずつ取り出して
足し合わせている状態は、その計算の規則が画面に漏れている。

```ts
// NG（ウォームアップを除く規則が画面ごとに必要になる）
let total = 0;
for (const set of sets) {
  if (!set.isWarmup) total += set.weightKg * set.reps;
}

// OK（規則は関数の中だけにある）
const { totalVolume } = summarizeSets(sets);
```
...
オブジェクト指向を前提とする原則のうち、次はこのリポジトリでは採らない。
「なぜ書かれていないか」を後から辿れるように残す。

| 原則 | 採らない理由 |
|---|---|
| static メソッドの誤用を避ける | クラスを使わない設計のため該当する構造が無い。「データと処理が離れている」問題は**高凝集・低結合**の節が受け持つ |
| 尋ねるな、命じろ | オブジェクトに振る舞いを持たせる前提の原則。関数型では**深く辿らない（デメテルの法則）**が同じ役割を果たす |
| モデリング・リファクタリング手法 | 設計プロセスの話であり、コーディング規約の範囲外。方針は `memory/roadmap.md` に置く |
