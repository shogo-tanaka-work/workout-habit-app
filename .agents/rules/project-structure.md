...
`types/` `db/` `hooks/` `utils/` `components/` `screens/` は技術の役割で切った構成で、
機能ごと（`workout/` `history/` `settings/` …）に切る構成とは別の考え方になる。

このリポジトリが役割別を採るのは、**1人が全機能を触る規模**だから。
機能別は「担当が分かれていて、機能をまたぐ変更が少ない」ときに効く。
いまは記録・履歴・設定が同じデータ構造を共有しており、機能で切ると
`workout/utils/datetime.ts` と `history/utils/datetime.ts` のような重複を生む。

ただし**役割別は「1つの機能を直すのに複数のディレクトリを開く」代償を払っている**。
画面が増えて役割別の各ディレクトリが膨らんできたら、機能別への切り替えを検討する。
その判断は roadmap に記録する。
