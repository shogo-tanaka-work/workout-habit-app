...
| 6 | モバイルの UI/UX 改善 | 監査で挙げた12件の課題を Phase 1〜5 の順に解消 | 進行中 |
...
| Phase | 課題 | 触る範囲 |
|---|---|---|
| 1 | #2 ウォームアップの集計 | `utils/aggregate.ts`、`apps/api/src/analytics.ts` |
| 2 | #4 RPE / #5 完了導線 / #11 入力確定 / #10 削除確認と復元 | `SetEditor` / `LabeledNumber` |
| 3 | #7 部位選択 → #8 種目編集 → #6 検索・絞り込み | `ExerciseScreen` / `WorkoutScreen` |
| 4 | #9 プリセットの上書き | **スキーマ変更**（D1・端末の両方＋同期対象） |
| 5 | #3 タイマー永続化 → #1 タイマー通知 | `useRestTimer`。**`expo-notifications` の新規導入** |
...
プリセット種目の上書き対象をどこまでにするか。レスト時間だけか、バー重量も含めるか。
**名前と部位は上書きの対象にしない**方針で設計する（共有プリセットの意味が失われるため）。
