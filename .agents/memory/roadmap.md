...
| 4-1 | D1 のテーブル再構築（`migrations/0002_multi_user_schema.sql`） | 実装済み・**本番未適用** |
| 4-2 | API の認証・認可・スコープ | 完了 |
| 4-3 | 操作ベース（intent）の CRUD エンドポイント | 未着手 |
| 4-4 | モバイル（Google サインイン・outbox・queries 書き換え） | 未着手 |
| 4-5 | 管理画面（Cloudflare Access 適用・member 区画） | 未着手 |

**本番 D1 への適用は 4-3 まで揃ってから。** 先に当てるとモバイルの `/backup` が
所有者列の NOT NULL 違反で失敗する。ローカル（`--local`）では適用・検証済み。
...
- 既存データは全行を所有者（`usr-owner` / admin）へ紐付ける。
  このユーザーは動作確認用も兼ねるため、画面表示に足りないサンプルは補う
